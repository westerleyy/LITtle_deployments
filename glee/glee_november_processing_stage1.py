...
for outlet in outlet:
    ...
    # EXPORT
    matched_ingredients_stock_in_df.to_csv(path + outlet + '\stock_in_ingredients_xwalk.csv', index = False)
    pd.DataFrame(recipe_ingredients_list).to_csv(path + outlet + '\\recipe_ingredients_list.csv', index = False)
    matched_recipe_pos_df.to_csv(path + outlet + '\\recipe_pos.csv', index = False)
    ...
    pd.DataFrame(unique_recipe_items).to_csv(path + outlet + '\\recipe_food_items_list.csv', index = False)
